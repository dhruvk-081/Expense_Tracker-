...
import { Card, CardBody, Text, Input, Select, Flex, Button } from "@chakra-ui/react";
...
const Filters = ({ filteredExpenses, setFilteredExpenses }) => {
  ...
  const handleFilters = () => {
    const newData = store.filter((expenseObj) => {
      ...
      else if (expenseObj.date.slice(5, 7) === month) return expenseObj;
      else if (expenseObj.date.slice(0, 4) === year) return expenseObj;
    });
      ...
      <CardBody>
        <Flex justifyContent="space-around">
          <Flex flexDirection="column" mr={2} ml={2}>
            ...
            <Input value={month} name="month" onChange={handleChange} size="md" />
          </Flex>
          <Flex flexDirection="column" mr={2} ml={2}>
